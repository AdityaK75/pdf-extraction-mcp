...
# Start MCP Server 2: Extractor, Embedder, Chunker, Vector Store
.venv/bin/python -m mcp.server \
  --tool server/pdf_extractor.py \
  --tool server/embedder.py \
  --tool server/chunker.py \
  --tool server/vector_store.py \
  --host 127.0.0.1 \
  --port 5002 \
  > logs/mcp_server_extract_embed_chunk_vector.log 2>&1 &
echo "Started MCP Server 2 (Extractor, Embedder, Chunker, Vector Store) on port 5002, logging to logs/mcp_server_extract_embed_chunk_vector.log" 